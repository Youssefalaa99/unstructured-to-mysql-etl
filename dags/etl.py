...
def extract_and_stage():
    ...
    for file_name in files:
        ...
        # Flatten logic 
        df_products = pd.json_normalize(records, record_path=['line_items'])[['product_id', 'sku', 'category', 'unit_price']].drop_duplicates()
        print(f"products:{df_products}")
        
        df_transactions = pd.json_normalize(records)[['transaction_id', 'timestamp', 'store.store_id', 'customer.customer_id', 'payment.method', 'payment.total_amount']]
        df_transactions.columns = ['transaction_id', 'timestamp', 'store_id', 'customer_id', 'payment_method', 'total_amount']
        print(f"transactions:{df_transactions}")

        df_stores = pd.json_normalize(records)[['store.store_id','store.location','store.region']]
        df_stores.columns = ['store_id','location','region']
        print(f"stores:{df_stores}")
        
        df_customers = pd.json_normalize(records)[['customer.customer_id','customer.personal_info.full_name','customer.personal_info.email','customer.loyalty_member']]
        df_customers.columns = ['customer_id','full_name','email','loyalty_member']
        ...
        df_customers = df_customers[['customer_id','first_name','last_name','email','loyalty_member']]
        print(f"customers: {df_customers}")

        df_transaction_items = pd.json_normalize(records, record_path=['line_items'],meta=['transaction_id'])[['product_id','quantity','transaction_id']]
